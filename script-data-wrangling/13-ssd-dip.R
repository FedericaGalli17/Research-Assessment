...
# Table 4.10:  Graduatoria dei Dipartimenti piccoli per SSD di afferenza dell'addetto
ssd_dip_p <- readxl::read_xlsx(nome_file,
                               sheet = "Tabella 4.10",
                               skip = 1)
sum(is.na(ssd_dip_p$SSD_add))
# Table 4.11:  Graduatoria dei Dipartimenti medi per SSD di afferenza dell'addetto
ssd_dip_m <- readxl::read_xlsx(nome_file,
                               sheet = "Tabella 4.11",
                               skip = 1)
sum(is.na(ssd_dip_m$SSD_add))
# Table 4.12:  Graduatoria dei Dipartimenti grandi per SSD di afferenza dell'addetto
...
ssd_dip$Dipartimento[is.na(ssd_dip$Istituzione)]
ssd_dip$Istituzione[is.na(ssd_dip$Istituzione)] <- "Palermo"
