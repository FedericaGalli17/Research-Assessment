...
area <- "04"
nome_file <- paste0("dati-vqr-11-14/VQR2011-2014_Area",area,"_Tabelle.xls")
# Table 4.6:  Graduatoria dei Dipartimenti piccoli per SSD di afferenza dell'addetto
ssd_dip_p <- readxl::read_xls(nome_file,
                           sheet = "4.6",
                           skip = 1)
sum(is.na(ssd_dip_p$SSD_add))

# Table 4.7:  Graduatoria dei Dipartimenti medi per SSD di afferenza dell'addetto
ssd_dip_m <- readxl::read_xls(nome_file,
                           sheet = "4.7",
                           skip = 1)
sum(is.na(ssd_dip_m$SSD_add))

# Table 4.8:  Graduatoria dei Dipartimenti grandi per SSD di afferenza dell'addetto
ssd_dip_g <- readxl::read_xls(nome_file,
                           sheet = "4.8",
                           skip = 1)
