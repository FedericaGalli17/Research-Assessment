...
# Table 4.2:  Graduatoria dei Dipartimenti piccoli 
dip_p <- readxl::read_xls(nome_file,
                           sheet = "4.2",
                           skip = 1)
# Table 4.3:  Graduatoria dei Dipartimenti medi 
dip_m <- readxl::read_xls(nome_file,
                           sheet = "4.3",
                           skip = 1)
dip_m <- dip_m %>%  filter(!is.na(Sottoistituzioni))

# Table 4.4:  Graduatoria dei Dipartimenti grandi 
dip_g <- readxl::read_xls(nome_file,
                           sheet = "4.4",
                           skip = 1)
...
rm(dip_p, dip_m, dip_g)

names(dip)[2] <- "Dipartimento"
