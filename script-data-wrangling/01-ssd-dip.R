...
# Table 4.8:  Graduatoria dei Dipartimenti piccoli per SSD di afferenza dell'addetto
ssd_dip_p <- readxl::read_xlsx(nome_file,
                           sheet = "Tabella 4.8",
                           skip = 5)
sum(is.na(ssd_dip_p$SSD_add))
ssd_dip_p$SSD_add <- zoo::na.locf(ssd_dip_p$SSD_add)
# Table 4.12:  Graduatoria dei Dipartimenti medi per SSD di afferenza dell'addetto
ssd_dip_m <- readxl::read_xlsx(nome_file,
                           sheet = "Tabella 4.12",
                           skip = 6)
sum(is.na(ssd_dip_m$SSD_add))
ssd_dip_m$SSD_add <- zoo::na.locf(ssd_dip_m$SSD_add)
# Table 4.16:  Graduatoria dei Dipartimenti grandi per SSD di afferenza dell'addetto
ssd_dip_g <- readxl::read_xlsx(nome_file,
                           sheet = "Tabella 4.16",
                           skip = 5)
sum(is.na(ssd_dip_g$SSD_add))
ssd_dip_g$SSD_add <- zoo::na.locf(ssd_dip_g$SSD_add)
