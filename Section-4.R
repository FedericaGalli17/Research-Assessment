...
indicator_labeller <- function(variable,value){
  return(indicator_names[value])
}

hist_data <- datadip %>% 
  ungroup() %>% 
  filter(np_dip > soglia) %>%
  select(ispd_d, ispd_star, r_dipstar, r_dip, I_dip) %>% 
  pivot_longer(cols = 1:5,names_to = "indicator", values_to = "value") 

hist_data$indicator_f <- factor(
  hist_data$indicator, 
  levels=c("ispd_d","ispd_star","I_dip", "r_dip", "r_dipstar")
)

h1 <- ggplot(hist_data, aes(x = value)) +
  geom_histogram(bins = 100, col = "black") +
  facet_grid(~indicator_f, scales = "free_x", labeller = indicator_labeller) +
  theme_bw(base_size = global_size) + ylab("") + xlab("") + 
  theme(strip.background = element_rect(
    fill="white"
  )) + scale_y_continuous(expand = expansion(0.02), labels = NULL) +
  theme(panel.spacing = unit(1.2, "lines"))
ggsave(file="plots-paper/provahist.pdf", h1, height = 3, width = 12)


#Figure 6: ispd vs ispdstar ---- 
norm_var_pc <- function(x){
  (x - min(x)) / (max(x) - min(x)) *100
} 
backtransf_norm_var_pc <- function(y, x){
  ((y / 100) * (max(x) - min(x))) + min(x)
} 

datadip_plot <- filter(datadip, np_dip > soglia)

soglie <- c(-0.1,5.1,94.9,100.1)
datadip_plot$classe_ispd <- as.numeric(cut(datadip_plot$ispd_d,soglie, include.lowest = FALSE))

indicators <- c("ispd_star", "I_dip", "r_dip", "r_dipstar")
lab_indicators <- c("ISPD*", "I", "R", "R*")

# ISPD vs ISPD*
sel_indicator <- 1
datadip_plot$compar_ind <- datadip_plot[[indicators[sel_indicator]]] 
datadip_plot$compar <- norm_var_pc(datadip_plot$compar_ind)

p_ispd_ispd_star <- datadip_plot %>% 
  filter(np_dip > soglia) %>% 
  filter(ispd > 95 | ispd < 5) %>%
  ggplot() + 
  geom_segment(aes(x = 0, y = ispd_d, xend = 1, yend = compar), linewidth = .1, 
               col = "lightgrey") +
  geom_point(aes(x = 0, y = ispd, col = as.factor(classe_ispd)), size = .75, show.legend = FALSE) + 
  geom_point(aes(x = 1, y = compar, col = as.factor(classe_ispd)), size = .75, show.legend = FALSE) + 
  xlab("") +
  scale_y_continuous("ISPD", sec.axis = 
                       sec_axis(
                         ~ backtransf_norm_var_pc(., datadip_plot$compar_ind), name = lab_indicators[sel_indicator]),
                     expand = expansion(0.03))+
  theme_bw(base_size = global_size) +
  scale_x_continuous(expand = expansion(0.01), breaks = c(0,1), labels = NULL)
ggsave(file="plots-paper/rankispdstar.pdf", p_ispd_ispd_star, height = 4, width = 6)
...
ggsave(file="plots-paper/rankR.pdf", p_ispd_R, height = 4, width = 6)
...
p1 <- ggplot(dipecc1, aes(Ispd, F4)) +
  ...
  theme_bw(base_size = global_size)+scale_size_area() +
  theme(legend.position = "none") +
  ylab("Project Score") + xlab("ISPD") + 
  ylim(c(16,30)) + xlim(c(76, 100)) +
  annotate("label", x = 80, y = 17, label = "2018-2022", size = 6)
p2 <- ggplot(dipecc2, aes(ispd, progetto)) +
  ...
  ylab("Project Score") + xlab("ISPD") + 
  ylim(c(16,30)) + xlim(c(76, 100)) +
  annotate("label", x = 80, y = 17, label = "2023-2027", size = 6)
p1m <- ggExtra::ggMarginal(p1, type = "histogram")
p2m <- ggExtra::ggMarginal(p2, type = "histogram")
gridExtra::grid.arrange(p1,p2, nrow = 1)
p <- gridExtra::arrangeGrob(p1m, p2m, nrow = 1)
ggsave(file="plots-paper/vardipecc.pdf", p, height = 5, width = 11)
